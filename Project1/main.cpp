...
// DirectX初期化処理 ここから
	...
	ConstBufferDataTransform* constMapTransform1 = nullptr;
	{
		...
		assert(SUCCEEDED(result));
		//定数バッファのマッピング
		result = constBuffTransform0->Map(0, nullptr,
	...
	//	imageData[i].x = (rand() % 100) / 100.0f;    // R
	//	imageData[i].y = (rand() % 100) / 100.0f;    // G
	//	imageData[i].z = (rand() % 100) / 100.0f;   // B
	//	imageData[i].w = (rand() % 100) / 100.0f;   // A
	//}
	TexMetadata metadata{};
	ScratchImage scratchImg{};
	// WICテクスチャのロード
	result = LoadFromWICFile(
		L"Resources/sin.jpg",   //「Resources」フォルダの「texture.png」
		WIC_FLAGS_NONE,
		&metadata, scratchImg);
	...
	textureResourceDesc.SampleDesc.Count = resDesc.MipLevels;;
	...
	XMMATRIX matworld;
	//ワールド
	matworld = constMapTransform0->mat = XMMatrixIdentity();
	//スケール
	XMMATRIX matScale;
	...
	matworld *= matTrans;
	constMapTransform0->mat = matView * matprojection * matworld;
...
// シェーダリソースビュー設定
	...
	srvDesc.Texture2D.MipLevels = 1; /*resDesc.mipLevels;*/
...
//ワールド変換行列
		...
		//ワールド行列を合成
		matworld1 = matScale1 * matRot1 * matTrans;
		//ワールド、ビュー、射影を合成
